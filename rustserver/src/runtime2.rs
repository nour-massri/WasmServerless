...
impl Runtime {
    ...
    pub fn new<P: AsRef<Path>>(cache_dir: P) -> Result<Self> {
        ...
        // I am getting this error here          ^^ expected `&mut WasiP1Ctx`, found `&mut WasmIOContext`
        wasmtime_wasi::preview1::add_to_linker_sync(&mut linker, |cx| cx.wasi_ctx())?;
    ...
    pub async fn create_instance(
        ...
        // Create the I/O context with environment variables
        let mut io_context = WasmIOContext::new(instance_id.clone(), env_vars, );
        
        // Set up WASI with command-line args
        // Create and build the WASI context
        let mut wasi_builder = WasiCtxBuilder::new();
        wasi_builder.inherit_stdio();
        
        // Add each argument individually to avoid using ? operator
        for arg in &args {
            wasi_builder.arg(arg);
        }
        
        let wasi = wasi_builder.build();
        io_context.set_wasi(wasi);
    ...
    pub async fn run_instance(&self, instance_id: &str) -> Result<(PerformanceMetrics, i32)> {
        ...
        let run_func = if let Some(func) = maybe_cached {
            ...
            let new_f = wasm_inst
                .get_typed_func::<(), i32>(&mut guard.store, "_run")
                .or_else(|_| wasm_inst.get_typed_func::<(), i32>(&mut guard.store, "_start"))
                .context("Failed to get entry function (_run or _start)")?;
